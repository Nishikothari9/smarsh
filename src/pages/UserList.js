...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(array, (_user) => (_user.name.toLowerCase().indexOf(query.toLowerCase()) !== -1 || _user.email.toLowerCase().indexOf(query.toLowerCase()) !== -1));
  }
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { id, name, role, status, company, avatarUrl, phone, empNo } = row;
                    ...
                    return (
                      <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        ...
                        {validValues && <TableCell align="right">
